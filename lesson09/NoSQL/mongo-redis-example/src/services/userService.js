...
const findUserById = async (userId) => {
  ...
  if (user) {
    await client.set(cacheKey, JSON.stringify(user), { EX: cacheTtl });
  }
  return user;
};
...
const createUser = async (data) => {
  ...
  await client.set(cacheKey, JSON.stringify(newUser), { EX: cacheTtl });
  return newUser;
};
...
const updateUser = async (data) => {
  ...
  if (updatedUser) {
    ...
    await client.set(cacheKey, JSON.stringify(updatedUser), { EX: cacheTtl });
  }
  return updatedUser;
};
...
const getUserComments = async (userId) => {
  ...
  await client.set(cacheKey, JSON.stringify(comments), { EX: cacheTtl });
  return comments;
};
